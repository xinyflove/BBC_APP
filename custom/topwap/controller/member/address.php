...
class topwap_ctl_member_address extends topwap_ctl_member{
    ...
    public function saveAddress()
    {
        ...
        {
            return $this->splash('error',null,'所在地区不能为空',true);
        }
        try{
        ...
        if ($validator->fails())
        {
            ...
            {
                return $this->splash('error',null,$e[0]);
            }
        }
        ...
        try
        {
            $insert_id = app::get('topwap')->rpcCall('user.address.add',$postdata);
        }
        catch(Exception $e)
        {
            $msg = $e->getMessage();
        ...
        $preg_res =preg_match('/addr_id/',$nextPage);
        if($preg_res)
        {
            $nextPage = preg_replace('/(.*)(addr_id=)(\d+)/','$1${2}'.$insert_id,$nextPage);
        }

        return $this->splash('success',$nextPage,$msg,true);
    }
    ...
    public function removeAddr()
    {
        ...
        if($postdata['mode'])
        {
            $nextPage = url::action('topwap_ctl_member_address@addrList',['mode'=>$postdata['mode']]);
        }else{
            $nextPage = url::action('topwap_ctl_member_address@addrList');
        }
        $postdata['user_id'] = userAuth::id();
