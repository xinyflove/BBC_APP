    ...
    */
    ...
    public function decode( $params )
    {
        ...
        foreach($params as $key=>$value )
        {
            ...
            $k =  $realKey ? $realKey : parseSearchKeyWord($key);

            if( is_array($value) )
            {
                foreach( $value as &$val )
                {
                    $val = parseSearchKeyWord($val);
                }
            }
            else
            {
                $value = parseSearchKeyWord($value);
            }

            $filter[$k] = is_array($value) ? implode(',', $value) : $value;
        }

        if($filter['orderBy'])
        {
            $filter['orderBy'] = $this->__filterOrderBy($filter['orderBy']);
        }
    ...
    //过滤排序
    private function __filterOrderBy($filter)
    {
        $array=array('order_sort','price','modified_time','list_time','sold_quantity','paid_quantity');
        $orderBy = explode(' ',$filter)[0];
        ...
        if(in_array($orderBy, $array))
        {
            switch ($orderBy) {
                ...
                case 'list_time':
                    ...
                    break;
                case 'price':
                    $orderByType = explode(' ',$filter)[1];
                    $orderByType = ($orderByType == 'desc') ? 'desc' : 'asc';
