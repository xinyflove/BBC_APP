...
class topwap_ctl_member_aftersales extends topwap_ctl_member {
    ...
    public function aftersalesApply()
    {
		...
		//取出供应商退货地址和退货手机号
		$pagedata['supplier']=$this->getSupplierInfo($oid);
    ...
    public function ajaxcreateAfterlogistics()
    {
		...
		/*add_2017/12/6_by_wanghaichao_start*/
		//取出oid 进而取出供应商信息
		$info=app::get('sysaftersales')->model('aftersales')->getRow('oid',array('aftersales_bn'=>$pagedata['aftersales_bn']));
		$pagedata['supplier']=$this->getSupplierInfo($info['oid']);
		/*add_2017/12/6_by_wanghaichao_end*/
	...
	* date 2017/11/14
	*/
	public function virtualAftersalesApply(){
        $pagedata = [];
        $tid = input::get('tid');
        $oid = input::get('oid');
        // 获取商品信息
	...
	* date 2017/11/15
	*/
	public function commitVirtualAftersalesApply(){
        $postdata = input::get();
		$postdata['aftersales_type']='ONLY_REFUND';
		$postdata['reason']='退款';
        $validator = validator::make(
            ['reason'=>$postdata['reason'],'description'=>$postdata['description'],'oid'=>$postdata['oid'],'aftersales_type'=>$postdata['aftersales_type'],'voucher_ids'=>$postdata['voucher_ids']],
            ['reason'=>'required','description'=>'|max:300','oid'=>'numeric','aftersales_type'=>'required','voucher_ids'=>'required'],
            ['reason'=>'售后申请理由必填!','description'=>'描述不能大于300','oid'=>'订单格式不对','aftersales_type'=>'请选择售后类型','voucher_ids'=>'请选择要退款的卡券']
        );
		$postdata['voucher_ids']=substr($postdata['voucher_ids'],0,-1);
        if ($validator->fails())
        {
            $messages = $validator->messagesInfo();
            foreach( $messages as $error )
        ...
        try
        {
            $result = app::get('topwap')->rpcCall('aftersales.apply', $postdata,'buyer');
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
    ...
    * date 2018/04/17
    */
    public function offlineVirtualAftersalesApply(){
        $pagedata = [];
        $tid = input::get('tid');
        $oid = input::get('oid');
        // 获取商品信息
        $filter['oid'] = $oid;
        $filter['fields'] = 'item_id,cat_id,end_time,bn,title,price,num,pic_path,spec_nature_info,gift_data';
        $pagedata['voucher'] = app::get('systrade')->model('agent_vocher')->getList('vocher_id',array('sys_oid'=>$oid,'status'=>'WAIT'));
        $pagedata['tid'] = $tid;
        $pagedata['oid'] = $oid;
        return $this->page('topwap/member/aftersales/offline_virtualapply.html' ,$pagedata);
    }
     ...
     */
	public function getSupplierInfo($oid){
        $filter['oid'] = $oid;
        $filter['fields'] = 'supplier_id';
        $orderInfo = app::get('topwap')->rpcCall('trade.order.get',$filter,'buyer');
		$supplier=app::get('sysshop')->model('supplier')->getRow('sh_phone,sh_address',array('supplier_id'=>$orderInfo['supplier_id'],'is_audit'=>'PASS'));
		return $supplier;
	}
