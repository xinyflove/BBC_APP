...
class sysactivityvote_vote_log {
    ...
    //     // $return = $objMdlVote->delete($params);
    //     $return = $objMdlVote->update(array('deleted' => 1),$params);
    //     if(!$return)
    //     {
    //         $result = false;
    //         $msg = "删除失败";
    ...
    {
        $objMdlVote = app::get('sysactivityvote')->model('vote');
        $vote = $objMdlVote->getList($row,$filter,$offset,$limit,$orderBy);
        return $vote;
    }

    public function countVoteLog($filter)
    {
        $objMdlVote = app::get('sysactivityvote')->model('vote');
        return $objMdlVote->count($filter);
    }
    ...
    // {
    //     $objMdlVote = app::get('sysactivityvote')->model('vote');
    //     $activity = $objMdlVote->getRow($row,$filter);
