...
class sysactivityvote_expert_comment {
    ...
    {
        $objMdlComment = app::get('sysactivityvote')->model('expert_comment');

        $result = true;

        // $return = $objMdlComment->delete($params);
        $return = $objMdlComment->update(array('deleted' => 1),$params);
        if(!$return)
        {
            $result = false;
            $msg = "删除失败";
    ...
    public function saveExpert($expertData)
    {
        ...
        try
        {
            if( !$objMdlComment->save($expertData) )
            {
    ...
    {
        $objMdlComment = app::get('sysactivityvote')->model('expert_comment');
        $comment = $objMdlComment->getList($row,$filter,$offset,$limit,$orderBy);
        return $comment;
    }

    public function countComment($filter)
    {
        $objMdlComment = app::get('sysactivityvote')->model('expert_comment');
        return $objMdlComment->count($filter);
    }
    ...
    {
        $objMdlComment = app::get('sysactivityvote')->model('expert_comment');
        $activity = $objMdlComment->getRow($row,$filter);
