...
class topshop_ctl_shop_article extends topshop_controller {
    ...
    private function __checkArticle($data)
    {
		...
		*/
		//文章标签改为必填
        $validator = validator::make(
                ['title' => $data['title'], 'content'=>$data['content'],'new_label'=>$data['new_label']],
                ['title' => 'required|min:2|max:50', 'content' => 'required|min:8','new_label'=>'required|max:3'],
                ['title' => '文章标题不能为空|文章标题至少两个字|文章标题最多五十字', 'content' => '文章内容不能为空|文章内容至少八个字','new_label' => '文章标签不能为空|文章内容最多三个字']
        );
        ...
        /*add_2017/10/26_by_fanglongji_start*/
        $params['new_label']=$data['new_label'];
        /*add_2017/10/26_by_fanglongji_end*/
     ...
     * 异步或取文章列表
     * @Author   王衍生
     * @DateTime 2017-09-21T14:00:23+0800
    ...
    public function ajaxArticleList()
    {
        ...
        $articleListData = app::get('topc')->rpcCall('syscontent.shop.list.article', $params);
        $count = $articleListData['count'];
        $pagedata['list'] = $articleListData['list'];

        //处理翻页数据
        $current = $filter['pages'] ? $filter['pages'] : 1;
        $pageSize = $filter['page_size'] ? $filter['page_size'] : 10;
        $filter['pages'] = time();
        if($count>0) $total = ceil($count/$pageSize);
        $pagedata['pagers'] = array(
            'link'=>url::action('topshop_ctl_shop_article@ajaxArticleList', $filter),
            'current'=>$current,
            'total'=>$total,
            'use_app'=>'topshop',
            'token'=>$filter['pages'],
        ...
        // return view::make('topshop/promotion/article/ajaxList.html', $pagedata);
        return view::make('topshop/shop/article/ajaxList.html', $pagedata);
    }
}
