...
class sysactivityvote_expert {
    ...
    public function saveExpert($expertData)
    {
        ...
        if($expertData['comment_password'])
        {
            // $objMdlExpert->count(arrar(''));
            if($expertData['expert_id'])
            {
                $count = $objMdlExpert->count(array('expert_id|notin' => $expertData['expert_id'], 'active_id' => $expertData['active_id'], 'comment_password' => $expertData['comment_password']));
            }
            else
            {

                $count = $objMdlExpert->count(array('active_id' => $expertData['active_id'], 'comment_password' => $expertData['comment_password']));

            }

            if($count)
            {
                throw new \LogicException('此密码已禁用,请更换点评密码！');
            }
        }
        ...
        try
        {
            if( !$objMdlExpert->save($expertData) )
            {
    ...
    {
        $objMdlExpert = app::get('sysactivityvote')->model('expert');
        $activity = $objMdlExpert->getRow($row,$filter);
