...
class topwap_item_search {
    ...
    public function search($filter)
    {
        ...
        if( empty($params['cat_id'])
            ...
            /*add_20171031_by_fanglongji_start*/
            && empty($params['shop_mold'])
            /*add_20171031_by_fanglongji_end*/
        )
        {
    ...
    public function setItemsActivetyTag($itemIds=null, $itemsList=null)
    {
		...
		/*修改代码*/
		$activityParams['start_time'] = 'sthan';
        $activityParams['fields'] = 'activity_id,item_id,activity_tag,price,activity_price,start_time,end_time';
    ...
    private function __preSearchFilter($params)
    {
		...
		$params['is_onsale']=$params['is_onsale']?$params['is_onsale']:'yes';
		if($params['is_onsale']=='yes'){
    ...
    /*add_2017-10-26by_xinyufeng_start*/
    /**
     * 根据搜索条件搜索出虚拟商品信息
     *
     * @poaram array $filter 搜索的条件
     * @auth xinyufeng
     */
    public function searchVirtual($filter)
    {
        //过滤搜索条件
        $objLibFilter = kernel::single('topwap_item_filter');
        $params = $objLibFilter->decode($filter);
        ...
        $this->__setActiveFilter($params);
        $params = $this->__preSearchFilter($params);
        $params['fields'] = $params['fields'].',is_virtual,show_mkt_price';//需要返回的商品字段
        $params['is_virtual'] = 1;
        $itemsList = app::get('topwap')->rpcCall('item.search',$params);
    ...
    /*add_2017-10-26by_xinyufeng_end*/
}
