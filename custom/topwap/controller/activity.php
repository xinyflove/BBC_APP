...
class topwap_ctl_activity extends topwap_controller{
    ...
    public function active_list()
    {
		...
		$weixin['descContent']= app::get('site')->getConf('site.activity_share_describe');
		$weixin['imgUrl']= base_storager::modifier($share_img);
		$weixin['linelink']= url::action("topwap_ctl_activity@active_list");
		$weixin['shareTitle']=app::get('site')->getConf('site.activity_share_title');
  ...
  //  }
    ...
    public function itemdetail()
    {
		...
		/*modify_2017/10/30_by_wanghaichao_start*/
        ...
        $pagedata['item'] = app::get('topwap')->rpcCall('item.get',array('item_id'=>$params['item_id'],'fields'=>'item_desc.wap_desc,item_store,item_status,sku,item_nature,spec_index,item_count.sold_quantity,item_count.item_id'));
		/*modify_2017/10/30_by_wanghaichao_end*/
        ...
        $pagedata ['defaultImageId'] = kernel::single('image_data_image')->getImageSetting('item');
         //店铺收藏的状态
        $pagedata['collect'] = $this->__CollectInfo($params['item_id'],$pagedata['group_item']['shop_id']);
		
        //echo "<pre>";print_r($pagedata);die();
        /*modify_2017/10/27_by_wanghaichao_start*/
        /*加载新的模板
        return $this->page("topwap/activity/activity_goods_detail.html",$pagedata);
		*/       
        return $this->page("topwap/activity/activity_goods_detail_new.html",$pagedata);
        /*modify_2017/10/27_by_wanghaichao_end*/
    }
    ...
    private function __getPagedata($post)
    {
        ...
        $params = array(
            ...
            'order_by' => $orderBy,
            'fields' => 'title,item_default_image,price,item_id,activity_id,sales_count,activity_price,shop_id',
        );
        if($post['id'])
        {
		...
		/*add_2017/10/31_by_wanghaichao_start*/
		//获取店铺的类型的,比如  电视啊 广播啊之类的
        $shop_infos = app::get('topc')->rpcCall('shop.list.all.get');
		foreach($item['list'] as $k=>&$items){			
		...
		/*add_2017/10/31_by_wanghaichao_end*/
