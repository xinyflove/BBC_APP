<?php
class topshop_ctl_trade_muumi_detail extends topshop_controller{
    public function index()
    {
        ...
        $this->runtimePath = array(
            ...
            ['url'=> url::action('topshop_ctl_trade_muumi_list@index'),'title' => app::get('topshop')->_('推送商品订单列表')],
            ['title' => app::get('topshop')->_('推送商品订单详情')],
        );
        ...
        */
        ...
        foreach($tradeInfo['orders'] as $key => $value){
            if($value['init_shop_id'] == $this->shopId){
                $total_fee += ($value['cost_price'] * $value['num']);
            }
        }
        // jj($tradeInfo['orders']);
        $tradeInfo['total_fee'] = $total_fee;
        ...
        if(in_array('CALL_CENTER_HOUSE', array_column($tradeInfo['orders'], 'source_house')) && ($tradeInfo['shipping_type'] == 'express')){
            $aggregation_filter = [
                'tid' => $params['tid'],
            ];
        ...
        return $this->page('topshop/trade/muumi/detail.html', $pagedata);
    }
    ...
    public function ajaxGetTrack()
    {
        ...
        return view::make('topshop/trade/muumi/trade_logistics.html',$pagedata);
    }
