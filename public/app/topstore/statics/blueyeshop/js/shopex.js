/**
 * Created by szr on 2017/8/28.
 */
/**
 * shopex核心JS
 */
var shopex = (function (document, undefined) {
    ...
    /**
     * shopex isWindow(需考虑obj为undefined的情况)
     */
    ...
    /**
     * shopex isPlainObject
     */
    $.isPlainObject = function (obj) {
        return $.isObject(obj) && !$.isWindow(obj) && Object.getPrototypeOf(obj) === Object.prototype;
    };
    /**
     * shopex isEmptyObject
     * @param {Object} o
     */
...
//window.shopex = shopex;
//'$' in window || (window.$ = shopex);
/**
 * $.os
 * @param {type} $
 * @returns {undefined}
...
(function ($, document) {
    function detect(ua) {
        ...
        if (plus) {
            this.os.plus = true;
            $(function () {
                document.body.classList.add('shopex-plus');
            });
            if (ua.match(/StreamApp/i)) { //TODO 最好有流应用自己的标识
                this.os.stream = true;
                $(function () {
                    document.body.classList.add('shopex-plus-stream');
                });
            }
        }
    }
...
(function ($) {
    ...
    var delegateFn = function (element, event, selector, callback) {
        return function (e) {
            ...
            for (; target && target !== document; target = target.parentNode) {
                ...
                if (~['click', 'tap', 'doubletap', 'longtap', 'hold'].indexOf(event) && (target.disabled || target.classList.contains('shopex-disabled'))) {
                    break;
                }
                var matches = {};
    ...
    $.fn.off = function (event, selector, callback) {
        return this.each(function () {
            var _mid = mid(this);
            if (!event) { //shopex(selector).off();
                delegates[_mid] && delete delegates[_mid];
            } else if (!selector) { //shopex(selector).off(event);
                delegates[_mid] && delete delegates[_mid][event];
            } else if (!callback) { //shopex(selector).off(event,selector);
                delegates[_mid] && delegates[_mid][event] && delete delegates[_mid][event][selector];
            } else { //shopex(selector).off(event,selector,callback);
...
/**
 * shopex target(action>popover>modal>tab>toggle)
 */
...
/**
 * shopex fixed requestAnimationFrame
 * @param {type} window
 * @returns {undefined}
 */
(function (window) {
...
(function ($, document) {
    $(function () {
        ...
        var CLASS_FOCUSIN = 'shopex-focusin';
        var CLASS_BAR_TAB = 'shopex-bar-tab';
        var CLASS_BAR_FOOTER = 'shopex-bar-footer';
        var CLASS_BAR_FOOTER_SECONDARY = 'shopex-bar-footer-secondary';
        var CLASS_BAR_FOOTER_SECONDARY_TAB = 'shopex-bar-footer-secondary-tab';
...
/**
 * shopex gesture flick[left|right|up|down]
 * @param {type} $
 * @param {type} name
 * @returns {undefined}
 */
...
/**
 * shopex gesture swipe[left|right|up|down]
 * @param {type} $
 * @param {type} name
 * @returns {undefined}
 */
...
/**
 * shopex gesture drag[start|left|right|up|down|end]
 * @param {type} $
 * @param {type} name
 * @returns {undefined}
 */
...
/**
 * shopex gesture tap and doubleTap
 * @param {type} $
 * @param {type} name
 * @returns {undefined}
 */
    ...
    var handle = function (event, touch) {
        ...
        switch (event.type) {
            case $.EVENT_END:
                ...
                if (!target || (target.disabled || (target.classList && target.classList.contains('shopex-disabled')))) {
                    return;
                }
...
(function ($) {
    ...
    var createIframe = function (options) {
        ...
        wrapper.className = 'shopex-iframe-wrapper';
    ...
    function handleScroll(wrapper, iframe) {
        var key = 'SHOPEX_SCROLL_POSITION_' + document.location.href + '_' + iframe.src;
    ...
    $(function () {
        ...
        if ($.os.ios) {
            ...
            classList.add('shopex-ios');
        } else if ($.os.android) {
            os.push({
                os: 'android',
                version: $.os.version
            });
            classList.add('shopex-android');
        }
        if ($.os.wechat) {
            os.push({
                os: 'wechat',
...
(function ($) {
    ...
    $.fire = function (webview, eventType, data) {
        if (webview) {
            ...
            webview.evalJS("typeof shopex!=='undefined'&&shopex.receive('" + eventType + "','" + data + "')");
        }
    };
    /**
     * 5+ event(5+没提供之前我自己实现)
    ...
    $.openWindow = function (url, id, options) {
        ...
        if (webviewCache && webview) { //已缓存
            //每次show都需要传递动画参数；
            //预加载的动画参数优先级：openWindow配置>preloadPages配置>shopex默认配置；
    ...
    $.addInit({
        ...
        handle: function () {
            ...
            if ($.os.plus) {
                ...
                //					err.className = 'shopex-error';
                //					//文字描述
                //					var span = document.createElement('span');
                //					span.innerHTML = '在该浏览器下，不支持创建子页面，具体参考';
                //					err.appendChild(span);
                //					var a = document.createElement('a');
                //					a.innerHTML = '"shopex框架适用场景"';
    ...
    $.ready(function () {
        ...
        if ($.supportStatusbarOffset()) {
            document.body.classList.add('shopex-statusbar');
        }
    });
})(shopex);
/**
...
(function ($, window) {
    ...
    window.addEventListener('tap', function (e) {
        ...
        if (action && action.classList.contains('shopex-action-back')) {
            $.back();
...
(function ($, window) {
    if ($.os.plus && $.os.android) {
        $.addBack({
            ...
            handle: function () {
                ...
                //popover
                if ($.targets._popover && $.targets._popover.classList.contains('shopex-active')) {
                    $($.targets._popover).popover('hide');
                    return true;
                }
                //offcanvas
                var offCanvas = document.querySelector('.shopex-off-canvas-wrap.shopex-active');
                if (offCanvas) {
    ...
    $.addBack({
        ...
        handle: function () {
            ...
            if (parent) {
                parent.evalJS('shopex&&shopex.back();');
            } else {
    ...
    $.menu = function () {
        var menu = document.querySelector('.shopex-action-menu');
        if (menu) {
            ...
            if (window.plus) {
                ...
                var parent = wobj.parent();
                if (parent) { //又得evalJS
                    parent.evalJS('shopex&&shopex.menu();');
                }
            }
        }
    };
                            ...
                            $.plusReady(function () {
                                ...
                                if (hasPulldown) {
                                    ...
                                    if (parent) {
                                        ...
                                        //父页面初始化pulldown
                                        parent.evalJS("shopex&&shopex(document.querySelector('.shopex-content')).pullRefresh('" + JSON.stringify(downOptions) + "')");
                                    }
                                }
                            });
                        } else {
...
/**
 * shopex layout(offset[,position,width,height...])
 * @param {type} $
 * @param {type} window
 * @param {type} undefined
 * @returns {undefined}
...
(function ($, document, undefined) {
    var CLASS_PULL_TOP_POCKET = 'shopex-pull-top-pocket';
    var CLASS_PULL_BOTTOM_POCKET = 'shopex-pull-bottom-pocket';
    var CLASS_PULL = 'shopex-pull';
    var CLASS_PULL_LOADING = 'shopex-pull-loading';
    var CLASS_PULL_CAPTION = 'shopex-pull-caption';
    var CLASS_PULL_CAPTION_DOWN = 'shopex-pull-caption-down';
    var CLASS_PULL_CAPTION_REFRESH = 'shopex-pull-caption-refresh';
    var CLASS_PULL_CAPTION_NOMORE = 'shopex-pull-caption-nomore';

    var CLASS_ICON = 'shopex-icon';
    var CLASS_SPINNER = 'shopex-spinner';
    var CLASS_ICON_PULLDOWN = 'shopex-icon-pulldown';

    var CLASS_BLOCK = 'shopex-block';
    var CLASS_HIDDEN = 'shopex-hidden';
    var CLASS_VISIBILITY = 'shopex-visibility';
    ...
    var Scroll = $.Class.extend({
        ...
        _initEvent: function (detach) {
            ...
            if (segmentedControl) { //靠，这个bug排查了一下午，阻止hash跳转，一旦hash跳转会导致可拖拽选项卡的tab不见
                shopex(segmentedControl)[detach ? 'off' : 'on']('click', 'a', $.preventDefault);
            }
...
(function ($, window, document, undefined) {
    ...
    var PullRefresh = $.Scroll.extend($.extend({
        ...
        disablePullupToRefresh: function () {
            this._initPullupRefresh();
            this.bottomPocket.className = 'shopex-pull-bottom-pocket' + ' ' + CLASS_HIDDEN;
...
(function ($, document) {
    ...
    var CLASS_PLUS_PULLREFRESH = 'shopex-plus-pullrefresh';
    var CLASS_VISIBILITY = 'shopex-visibility';
    var CLASS_HIDDEN = 'shopex-hidden';
    var CLASS_BLOCK = 'shopex-block';
    ...
    var PlusPullRefresh = $.Class.extend({
                    ...
                    webview.addEventListener("dragBounce", function (e) {
                        ...
                        switch (e.status) {
                            ...
                            case "dragEndAfterChangeOffset": //正在刷新状态
                                ...
                                webview.evalJS("shopex&&shopex.options.pullRefresh.down.callback()");
        ...
        //			var self = this;
        //			$.plusReady(function() {
        //				plus.webview.getWebviewById(self.options.webviewId).evalJS("shopex&&shopex.options.pullRefresh.down&&shopex.options.pullRefresh.down.callback()");
        //			});
        //		},
        endPulldownToRefresh: function () { //该方法是子页面调用的
            var webview = plus.webview.currentWebview();
            webview.parent().evalJS("shopex&&shopex(document.querySelector('.shopex-content')).pullRefresh('" + JSON.stringify({
...
(function ($, window, document, name) {
    var CLASS_OFF_CANVAS_LEFT = 'shopex-off-canvas-left';
    var CLASS_OFF_CANVAS_RIGHT = 'shopex-off-canvas-right';
    var CLASS_ACTION_BACKDROP = 'shopex-off-canvas-backdrop';
    var CLASS_OFF_CANVAS_WRAP = 'shopex-off-canvas-wrap';

    var CLASS_SLIDE_IN = 'shopex-slide-in';
    var CLASS_ACTIVE = 'shopex-active';


    var CLASS_TRANSITIONING = 'shopex-transitioning';

    var SELECTOR_INNER_WRAP = '.shopex-inner-wrap';
    ...
    var OffCanvas = $.Class.extend({
        init: function (element, options) {
            ...
            if (this.scroller) {
                ...
                document.body.classList.add('shopex-fullscreen'); //fullscreen
                this.refresh();
                this.initEvent();
            }
        },
        ...
        refresh: function (offCanvas) {
            ...
            this.scalable = this.classList.contains('shopex-scalable') && !this.slideIn;
            this.scroller = this.wrapper.querySelector(SELECTOR_INNER_WRAP);
        ...
        initEvent: function () {
            ...
            if (this.classList.contains('shopex-draggable')) {
    ...
    $.ready(function () {
        $('.shopex-off-canvas-wrap').offCanvas();
    });
})(shopex, window, document, 'offcanvas');
/**
 * actions
 * @param {type} $
 * @param {type} name
 ...
 */
(function ($, name) {
    var CLASS_ACTION = 'shopex-action';
    ...
    var handle = function (event, target) {
        ...
        if (className && ~className.indexOf(CLASS_ACTION)) {
            if (target.classList.contains('shopex-action-back')) {
                event.preventDefault();
            }
            return target;
        }
...
(function ($, window, document, name) {
    ...
    window.addEventListener('tap', function (event) {
        if ($.targets.modal) {
            ...
            $.targets.modal.classList.toggle('shopex-active');
        }
    });
})(shopex, window, document, 'modal');
/**
 * Popovers
 * @param {type} $
 * @param {type} window
...
(function ($, window, document, name) {
    ...
    var CLASS_POPOVER = 'shopex-popover';
    var CLASS_POPOVER_ARROW = 'shopex-popover-arrow';
    var CLASS_ACTION_POPOVER = 'shopex-popover-action';
    var CLASS_BACKDROP = 'shopex-backdrop';
    var CLASS_BAR_POPOVER = 'shopex-bar-popover';
    var CLASS_BAR_BACKDROP = 'shopex-bar-backdrop';
    var CLASS_ACTION_BACKDROP = 'shopex-backdrop-action';
    var CLASS_ACTIVE = 'shopex-active';
    var CLASS_BOTTOM = 'shopex-bottom';
    ...
    var togglePopover = function (popover, anchor, state) {
        ...
        var _popover = document.querySelector('.shopex-popover.shopex-active');
        if (_popover) {
...
})(shopex, window, document, 'popover');
/**
...
(function ($, window, name) {
    ...
    var CLASS_SWITCH = 'shopex-switch';
    var CLASS_SWITCH_HANDLE = 'shopex-switch-handle';
    var CLASS_ACTIVE = 'shopex-active';
    var CLASS_DRAGGING = 'shopex-dragging';

    var CLASS_DISABLED = 'shopex-disabled';
...
(function ($, window, document) {
    ...
    var CLASS_ACTIVE = 'shopex-active';
    var CLASS_SELECTED = 'shopex-selected';
    var CLASS_GRID_VIEW = 'shopex-grid-view';
    var CLASS_RADIO_VIEW = 'shopex-table-view-radio';
    var CLASS_TABLE_VIEW_CELL = 'shopex-table-view-cell';
    var CLASS_COLLAPSE_CONTENT = 'shopex-collapse-content';
    var CLASS_DISABLED = 'shopex-disabled';
    var CLASS_TOGGLE = 'shopex-switch';
    var CLASS_BTN = 'shopex-btn';

    var CLASS_SLIDER_HANDLE = 'shopex-slider-handle';
    var CLASS_SLIDER_LEFT = 'shopex-slider-left';
    var CLASS_SLIDER_RIGHT = 'shopex-slider-right';
    var CLASS_TRANSITIONING = 'shopex-transitioning';
    ...
    var radioOrCheckboxClick = function (event) {
        ...
        if (classList.contains('shopex-radio')) {
        ...
        } else if (classList.contains('shopex-checkbox')) {
    ...
    //fixed hashchange(android)
    window.addEventListener($.EVENT_CLICK, function (e) {
        if (cell && cell.classList.contains('shopex-collapse')) {
            e.preventDefault();
        }
    });
    ...
    window.addEventListener('tap', function (event) {
        ...
        if (classList.contains('shopex-collapse') && !cell.parentNode.classList.contains('shopex-unfold')) {
            if (!preventDefaultException.test(event.target.tagName)) {
                event.detail.gesture.preventDefault();
            }

            if (!classList.contains(CLASS_ACTIVE)) { //展开时,需要收缩其他同类
                var collapse = cell.parentNode.querySelector('.shopex-collapse.shopex-active');
                if (collapse) {
...
(function ($, window) {
     ...
     * 自动消失提示框
     */
    $.toast = function (message, callback, delay) {
        if ($.os.plus) {
            //默认显示在底部；
            $.plusReady(function () {
            ...
            toast.classList.add('shopex-toast-container');
            toast.innerHTML = '<div class="' + 'shopex-toast-message' + '">' + message + '</div>';
            ...
            }, typeof delay === 'number' && delay * 1000 || 2000);
        }
    };
...
(function ($, document) {
    ...
    /**
     * 创建并显示进度条
     * @param {Object} container  可选，默认body，支持selector,DOM Node,shopex wrapper
    ...
    $.fn.progressbar = function (options) {
        ...
        this.each(function () {
            var self = this;
            var progressbarApi = self.shopex_plugin_progressbar;
            if (!progressbarApi) {
                self.shopex_plugin_progressbar = progressbarApi = {
                    options: options,
...
(function ($) {
    ...
    var holderClassName = 'shopex-numbox';
    var plusClassSelector = '.shopex-btn-numbox-plus,.shopex-numbox-btn-plus';
    var minusClassSelector = '.shopex-btn-numbox-minus,.shopex-numbox-btn-minus';
    var inputClassSelector = '.shopex-input-numbox,.shopex-numbox-input';
    ...
    //自动处理 class='shopex-locker' 的 dom
    $.ready(function () {
...
(function ($, window) {
    ...
    var Transparent = function (element, options) {
        ...
        var color = getColor(shopex.getStyles(this.element, 'backgroundColor'));
        if (color.length) {
            this._R = color[0];
            this._G = color[1];
            this._B = color[2];
    ...
    Transparent.prototype.destory = function () {
        ...
        this.element.shopex_plugin_transparent = null;
    };
    $.fn.transparent = function (options) {
        options = options || {};
        var transparentApis = [];
        this.each(function () {
            var transparentApi = this.shopex_plugin_transparent;
            if (!transparentApi) {
                ...
                transparentApi = this.shopex_plugin_transparent = new Transparent(this, options);
            }
